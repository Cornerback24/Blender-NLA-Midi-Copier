...
def generate_transition_strip(strip1, strip2, nla_track, interpolation, easing, frame_length_limit: int = None,
    ...
    if len(fcurves_to_generate) > 0:
        action = new_action(f"{strip1.action.name} Transition")
...
def new_action(name: str):
    return bpy.context.blend_data.actions.new(name)
